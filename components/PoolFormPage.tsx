...
const PoolFormPage: React.FC<PoolFormPageProps> = ({ onSave, onCancel, initialData }) => {
  ...
  const handleSearchLocation = async () => {
    ...
    try {
      ...
      if (results.length === 0) {
          alert("AI 검색으로 정보를 찾지 못했습니다. '주소 검색' 버튼을 사용해 주세요.");
      }
    } catch (error) {
      console.error(error);
    } finally {
  ...
  const handleManualAddressSearch = () => {
    ...
    new (window as any).daum.Postcode({
      oncomplete: async (data: any) => {
        ...
        else setSelectedCoords({ lat: 37.5665, lng: 126.9780 }); // Fallback to Seoul
    ...
    <div className="min-h-screen bg-[#F8FAFC]">
      {(isSubmitting || isSearching) && (
        <div className="fixed inset-0 z-[100] bg-white/80 backdrop-blur-sm flex flex-col items-center justify-center">
          ...
          <p className="font-bold text-slate-800 text-center px-6">AI가 정보를 확인 중입니다...</p>
        </div>
      )}
        ...
        <div className="bg-amber-50 border-b border-amber-200 py-3 px-4 animate-in slide-in-from-top duration-500">
          <div className="max-w-4xl mx-auto flex items-center justify-between gap-4">
            ...
            <div className="flex gap-2">
              <button onClick={clearDraft} className="text-xs text-amber-600 font-bold px-3 py-1.5">삭제</button>
      ...
      <div className="bg-white border-b border-slate-200 py-4 px-4 sticky top-0 z-30 shadow-sm">
        <div className="max-w-4xl mx-auto flex items-center justify-between">
          ...
          <button type="submit" form="pool-form" className="bg-[#2563EB] text-white px-8 py-2.5 rounded-full font-bold shadow-md">저장</button>
        </div>
      </div>
        ...
        <form id="pool-form" onSubmit={handleSubmit} className="space-y-8">
            ...
            <div className="space-y-4">
                <div className="flex flex-col sm:flex-row gap-2">
                    ...
                    <div className="flex gap-2 shrink-0">
                      <button type="button" onClick={handleSearchLocation} className="flex-1 sm:flex-none bg-slate-800 text-white px-6 rounded-2xl font-bold hover:bg-slate-900 shadow-md transition-all flex items-center justify-center gap-2">
                          <Search className="w-5 h-5" /> AI 검색
                      </button>
                      <button type="button" onClick={handleManualAddressSearch} className="flex-1 sm:flex-none bg-white border border-slate-200 text-slate-700 px-6 rounded-2xl font-bold shadow-sm transition-all flex items-center justify-center gap-2">
                          주소 검색
                      </button>
                    </div>
                </div>
                    ...
                    <div className="border border-slate-100 rounded-2xl overflow-hidden shadow-xl bg-white relative z-40 max-h-80 overflow-y-auto divide-y divide-slate-50 animate-in fade-in slide-in-from-top-2">
                        {searchResults.map((result, idx) => (
                            <div key={idx} onClick={() => handleSelectLocation(result)} className="p-5 hover:bg-blue-50 cursor-pointer transition-colors group">
                                <div className="font-bold text-slate-800 group-hover:text-blue-600 text-base">{result.name}</div>
                ...
                {address && (
                    <div className="bg-gradient-to-r from-blue-50 to-indigo-50 border border-blue-100 p-6 rounded-2xl shadow-inner">
          ...
          <div className="bg-white p-8 rounded-2xl border border-slate-200 shadow-sm space-y-6">
            <div className="flex justify-between items-center border-b border-slate-100 pb-4">
                ...
                <button type="button" onClick={addSchedule} className="flex items-center gap-1.5 bg-blue-50 text-blue-600 px-4 py-2 rounded-xl text-sm font-bold">
                ...
                {schedules.map((sch, idx) => (
                    <div key={idx} className="flex flex-col md:flex-row gap-3 p-5 bg-slate-50 rounded-2xl border border-slate-100">
                        <div className="flex-1 md:flex-[0.5] relative">
                            <select value={sch.day} onChange={e => updateSchedule(idx, 'day', e.target.value)} className="w-full pl-4 pr-10 py-3.5 bg-white border border-slate-200 rounded-xl text-sm font-bold outline-none">
          ...
          <div className="bg-white p-8 rounded-2xl border border-slate-200 shadow-sm space-y-6">
            <div className="flex justify-between items-center border-b border-slate-100 pb-4">
                <h3 className="text-xl font-bold text-slate-900 flex items-center gap-2">
                    <DollarSign className="w-6 h-6 text-blue-500" /> 이용 요금
                </h3>
                <button type="button" onClick={addFee} className="flex items-center gap-1.5 bg-blue-50 text-blue-600 px-4 py-2 rounded-xl text-sm font-bold">
                ...
                {fees.map((fee, idx) => (
                    <div key={idx} className="flex flex-col md:flex-row gap-3 p-5 bg-slate-50 rounded-2xl border border-slate-100">
                        <div className="flex-1 flex gap-2">
                            <select value={fee.category} onChange={e => updateFee(idx, 'category', e.target.value)} className="w-full p-3.5 bg-white border border-slate-200 rounded-xl text-sm font-bold outline-none">
                                <option value="평일">평일</option>
                                <option value="주말/공휴일">주말/공휴일</option>
                            </select>
                            <select value={fee.type} onChange={e => updateFee(idx, 'type', e.target.value)} className="w-full p-3.5 bg-white border border-slate-200 rounded-xl text-sm font-bold outline-none">
                        ...
                        <div className="flex-1 flex gap-3 items-center">
                            <input type="number" value={fee.price} onChange={e => updateFee(idx, 'price', Number(e.target.value))} className="w-full p-3.5 bg-white border border-slate-200 rounded-xl text-sm font-bold" placeholder="요금 입력" />
          ...
          <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
            ...
            <div className="divide-y divide-slate-100 p-8 space-y-6">
                <div className="flex items-center gap-3">
                    <button type="button" onClick={() => setRegularHolidayEnabled(!regularHolidayEnabled)} className={`relative w-12 h-6 rounded-full transition-colors ${regularHolidayEnabled ? 'bg-[#3b82f6]' : 'bg-slate-300'}`}><div className={`absolute top-1 left-1 w-4 h-4 bg-white rounded-full transition-transform ${regularHolidayEnabled ? 'translate-x-6' : 'translate-x-0'}`}></div></button>
                    <span className="text-sm font-bold text-slate-600">정기 휴무일 설정</span>
                </div>
                {regularHolidayEnabled && (
                    <div className="space-y-4">
                        {holidayRules.map((rule, idx) => (
                            <div key={idx} className="flex flex-wrap gap-2 items-center">
                                <select value={rule.weekNumber} onChange={e => updateHolidayRule(idx, 'weekNumber', Number(e.target.value))} className="p-3 bg-white border border-slate-200 rounded-xl text-sm font-bold outline-none shadow-sm">
                                    {WEEK_NUMBERS.map(wn => <option key={wn.value} value={wn.value}>{wn.label}</option>)}
                                </select>
                                <select value={rule.dayOfWeek} onChange={e => updateHolidayRule(idx, 'dayOfWeek', Number(e.target.value))} className="p-3 bg-white border border-slate-200 rounded-xl text-sm font-bold outline-none shadow-sm">
                                    {WEEK_DAYS.map((day, dIdx) => <option key={dIdx} value={dIdx}>{day}</option>)}
                                </select>
                                <button type="button" onClick={() => removeHolidayRule(idx)} className="p-3 text-[#ff4d4d] border border-red-200 rounded-xl"><Trash2 className="w-4 h-4" /></button>
                            </div>
                        ))}
                        <button type="button" onClick={addHolidayRule} className="px-6 py-3 border-2 border-dashed border-blue-200 rounded-xl text-blue-600 text-sm font-bold hover:bg-blue-50 transition-colors">+ 추가</button>
                    </div>
                )}
            </div>
          </div>
          ...
          <div className="bg-white p-8 rounded-2xl border border-slate-200 shadow-sm space-y-8">
            <h3 className="text-xl font-bold text-slate-900 flex items-center gap-2 border-b border-slate-100 pb-4">
                <Info className="w-6 h-6 text-blue-500" /> 운영 및 시설 상세
            </h3>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                <div className="space-y-4">
                    <label className="block text-xs font-bold text-slate-400 mb-2 ml-1">레인 정보</label>
                    <div className="flex gap-4">
                        <input type="number" value={lanes} onChange={e => setLanes(Number(e.target.value))} className="w-full p-4 bg-slate-50 border border-slate-200 rounded-2xl font-bold" placeholder="레인 수" />
                        <input type="number" value={length} onChange={e => setLength(Number(e.target.value))} className="w-full p-4 bg-slate-50 border border-slate-200 rounded-2xl font-bold" placeholder="길이(m)" />
                    </div>
                </div>
                <div className="space-y-4">
                    <label className="block text-xs font-bold text-slate-400 mb-2 ml-1">이미지 URL</label>
            ...
            <div>
                <label className="block text-sm font-bold text-slate-700 mb-3 ml-1">기타 안내 (꿀팁)</label>
                <textarea value={extraFeatures} onChange={e => setExtraFeatures(e.target.value)} className="w-full p-6 bg-slate-50 border border-slate-200 rounded-2xl outline-none min-h-[120px] text-sm leading-relaxed focus:bg-white transition-all" placeholder="오리발 사용 가능 시간 등..." />
            </div>
          </div>
        </form>
      </main>
